...
bool EmulatorWindow::Initialize() {
  ...
  // Network menu
  auto network_menu = MenuItem::Create(MenuItem::Type::kPopup, "&Network");
  {
    network_menu->AddChild(MenuItem::Create(
        MenuItem::Type::kString, "&Netplay Settings", "",
        std::bind(&EmulatorWindow::ToggleNetplayConfigDialog, this)));
  }
  main_menu->AddChild(std::move(network_menu));
...
void EmulatorWindow::ToggleNetplayConfigDialog() {
  if (!netplay_config_dialog_) {
    netplay_config_dialog_ = std::unique_ptr<NetplayConfigDialog>(
        new NetplayConfigDialog(imgui_drawer_.get(), *this));
  } else {
    netplay_config_dialog_.reset();
  }
}

EmulatorWindow::NetplayConfigDialog::NetplayConfigDialog(
    ui::ImGuiDrawer* imgui_drawer, EmulatorWindow& emulator_window)
    : ui::ImGuiDialog(imgui_drawer), emulator_window_(emulator_window) {
  network_mode_ = cvars::network_mode;
  upnp_enabled_ = cvars::upnp;
  xstorage_enabled_ = cvars::xstorage_backend;
  logging_enabled_ = cvars::logging;
  mask_ips_ = cvars::log_mask_ips;
  RefreshServerList();
  RefreshStatus();

  friends_list_ = kernel::XLiveAPI::ParseFriendsXUIDs();
}

void EmulatorWindow::NetplayConfigDialog::RefreshServerList() {
  server_list_ = kernel::XLiveAPI::ParseAPIList();
  std::string current = kernel::XLiveAPI::GetApiAddress();
  selected_server_ = 0;
  for (size_t i = 0; i < server_list_.size(); i++) {
    if (server_list_[i] == current) {
      selected_server_ = static_cast<int>(i);
      break;
    }
  }
}

void EmulatorWindow::NetplayConfigDialog::RefreshStatus() {
  auto state = kernel::XLiveAPI::GetInitState();
  if (state == kernel::XLiveAPI::InitState::Success) {
    status_text_ = "Connected";
    local_ip_text_ = kernel::XLiveAPI::LocalIP_str();
    online_ip_text_ = kernel::XLiveAPI::OnlineIP_str();
  } else if (state == kernel::XLiveAPI::InitState::Pending) {
...
void EmulatorWindow::NetplayConfigDialog::OnDraw(ImGuiIO& io) {
  ImGui::SetNextWindowPos(ImVec2(40, 40), ImGuiCond_FirstUseEver);
  ImGui::SetNextWindowSize(ImVec2(500, 0), ImGuiCond_FirstUseEver);

  bool dialog_open = true;
  if (!ImGui::Begin("Netplay Settings", &dialog_open,
                    ImGuiWindowFlags_NoCollapse |
                        ImGuiWindowFlags_AlwaysAutoResize)) {
    Close();
    ImGui::End();
  ...
  // === Network Mode ===
  ImGui::SeparatorText("Network Mode");
  {
    const char* modes[] = {"Offline", "Systemlink", "Xbox Live"};
    if (ImGui::Combo("Mode", &network_mode_, modes, IM_ARRAYSIZE(modes))) {
      kernel::XLiveAPI::SetNetworkMode(static_cast<uint32_t>(network_mode_));
      config_changed = true;
      RefreshStatus();
    }
    ...
    switch (network_mode_) {
      case 0:
        ImGui::TextDisabled("No networking. Games run in offline mode.");
        break;
      case 1:
        ImGui::TextDisabled(
            "LAN play. Connect to other Xenia instances on your "
            "local network.");
        break;
      case 2:
        ImGui::TextDisabled(
            "Online play via Xenia-WebServices backend server.");
        break;
    }
  }

  // === Server Selection (only for Xbox Live mode) ===
  bool is_online_mode = (network_mode_ == 2);
  if (is_online_mode) {
    ImGui::SeparatorText("Server");
    {
      if (!server_list_.empty()) {
        std::string combo_preview = server_list_[selected_server_];
        if (ImGui::BeginCombo("Server", combo_preview.c_str())) {
          for (int i = 0; i < static_cast<int>(server_list_.size()); i++) {
            bool is_selected = (selected_server_ == i);
            if (ImGui::Selectable(server_list_[i].c_str(), is_selected)) {
              selected_server_ = i;
              kernel::XLiveAPI::SetAPIAddress(server_list_[i]);
              config_changed = true;
              RefreshStatus();
            }
            if (is_selected) {
      ...
      ImGui::InputText("Custom", custom_server_,
                       IM_ARRAYSIZE(custom_server_));
      ImGui::SameLine();
      if (ImGui::Button("Connect")) {
        std::string addr(custom_server_);
        if (!addr.empty()) {
          kernel::XLiveAPI::SetAPIAddress(addr);
          config_changed = true;
          RefreshServerList();
          RefreshStatus();
        }
  ...
  ImGui::SeparatorText("Status");
  {
    ...
    if (state == kernel::XLiveAPI::InitState::Success) {
      ...
      ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "Not connected");
    } else {
      ImGui::TextColored(ImVec4(1.0f, 0.3f, 0.3f, 1.0f), "Failed");
    }

    if (!local_ip_text_.empty()) {
      ImGui::Text("Local IP:  %s", local_ip_text_.c_str());
    }
    if (!online_ip_text_.empty()) {
      ImGui::Text("Online IP: %s", online_ip_text_.c_str());
    }

    if (network_mode_ == 0) {
      ImGui::TextDisabled("Offline mode — no connection needed.");
    } else if (state == kernel::XLiveAPI::InitState::Pending &&
               is_online_mode) {
      if (ImGui::Button("Connect Now")) {
        kernel::XLiveAPI::Init();
        RefreshStatus();
      }
      ImGui::SameLine();
      ImGui::TextDisabled(
          "Connects to the server. Also happens automatically when a "
          "game makes a network call.");
    } else if (state == kernel::XLiveAPI::InitState::Failed &&
               is_online_mode) {
      ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.0f, 1.0f),
                         "Connection failed. Check server address and "
                         "network connectivity.");
      if (ImGui::Button("Retry Connection")) {
        kernel::XLiveAPI::ResetInitState();
        kernel::XLiveAPI::Init();
        RefreshStatus();
      }
    } else if (state == kernel::XLiveAPI::InitState::Pending &&
               network_mode_ == 1) {
      ImGui::TextDisabled(
          "Systemlink mode — no backend server needed. Games connect "
          "directly over LAN.");
    }
  }

  // === Friends (only for Xbox Live mode) ===
  if (is_online_mode) {
    ImGui::SeparatorText("Friends");
    {
      ImGui::InputText("XUID", friend_xuid_input_,
                       IM_ARRAYSIZE(friend_xuid_input_),
                       ImGuiInputTextFlags_CharsDecimal);
      ImGui::SameLine();
      if (ImGui::Button("Add Friend")) {
        char* end = nullptr;
        uint64_t xuid = std::strtoull(friend_xuid_input_, &end, 10);
        if (xuid != 0) {
          kernel::XLiveAPI::AddFriend(xuid);
          friends_list_ = kernel::XLiveAPI::ParseFriendsXUIDs();
          friend_xuid_input_[0] = '\0';
          config_changed = true;
        }
      }

      if (!friends_list_.empty()) {
        ImGui::Text("Friends (%zu):", friends_list_.size());
        int remove_idx = -1;
        for (size_t i = 0; i < friends_list_.size(); i++) {
          ImGui::PushID(static_cast<int>(i));
          ImGui::Text("  %016llX",
                       static_cast<unsigned long long>(friends_list_[i]));
          ImGui::SameLine();
        ...
        if (remove_idx >= 0) {
          kernel::XLiveAPI::RemoveFriend(friends_list_[remove_idx]);
          friends_list_ = kernel::XLiveAPI::ParseFriendsXUIDs();
          config_changed = true;
        }
      } else {
  ...
  ImGui::SeparatorText("Options");
  {
    if (is_online_mode) {
      ...
      if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "Automatically forward ports on your router for direct "
  ...
  // Save config to disk whenever something changes
  if (config_changed) {
    config::SaveConfig();
  }
  ...
  if (!dialog_open) {
    Close();
    emulator_window_.netplay_config_dialog_.release();
    return;
  }
}
