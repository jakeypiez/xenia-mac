...
void DumpGeometryArtifact(const char* dump_dir, int dump_id,
  ...
  std::snprintf(filename, sizeof(filename), "%s/geometry_%d_%s_%08X_%s",
                dump_dir, dump_id, GetGeometryShaderTypeName(type), key,
                suffix);
...
void ProbeGeometryMetallib(MTL::Device* device) {
  ...
  fc->setConstantValue(
      &tessellation_enabled, MTL::DataTypeBool,
...
MetalCommandProcessor::~MetalCommandProcessor() {
  ...
  if (render_target_texture_) {
    TrackMetalTextureReleased(render_target_texture_);
    render_target_texture_->release();
    render_target_texture_ = nullptr;
  }
  if (depth_stencil_texture_) {
    TrackMetalTextureReleased(depth_stencil_texture_);
  ...
  if (null_buffer_) {
    TrackMetalBufferReleased(null_buffer_->length());
    null_buffer_->release();
    null_buffer_ = nullptr;
  }
  if (null_texture_) {
    TrackMetalTextureReleased(null_texture_);
...
MetalCommandProcessor::DrawRingBuffers::~DrawRingBuffers() {
  if (res_heap_ab) {
    TrackMetalBufferReleased(res_heap_ab->length());
    res_heap_ab->release();
    res_heap_ab = nullptr;
  }
  if (smp_heap_ab) {
    TrackMetalBufferReleased(smp_heap_ab->length());
    smp_heap_ab->release();
    smp_heap_ab = nullptr;
  }
  if (cbv_heap_ab) {
    TrackMetalBufferReleased(cbv_heap_ab->length());
    cbv_heap_ab->release();
    cbv_heap_ab = nullptr;
  }
  if (uniforms_buffer) {
    TrackMetalBufferReleased(uniforms_buffer->length());
    uniforms_buffer->release();
    uniforms_buffer = nullptr;
  }
  if (top_level_ab) {
    TrackMetalBufferReleased(top_level_ab->length());
    top_level_ab->release();
    top_level_ab = nullptr;
  }
  if (draw_args_buffer) {
    TrackMetalBufferReleased(draw_args_buffer->length());
...
bool MetalCommandProcessor::SetupContext() {
  ...
  if (mesh_shader_supported_) {
    ...
    TrackMetalBufferCreated(tess_tables_size);
  ...
  TrackMetalTextureCreated(render_target_texture_);
  ...
  TrackMetalTextureCreated(depth_stencil_texture_);
  ...
  TrackMetalBufferCreated(kNullBufferSize);
  ...
  TrackMetalTextureCreated(null_texture_);
...
bool MetalCommandProcessor::InitializeShaderTranslation() {
  ...
  shader_translator_ = std::make_unique<DxbcShaderTranslator>(
      ...
      false, ::cvars::gamma_render_target_as_srgb,
...
void MetalCommandProcessor::ShutdownContext() {
  ...
  if (tessellator_tables_buffer_) {
    TrackMetalBufferReleased(tessellator_tables_buffer_->length());
...
void MetalCommandProcessor::CaptureCurrentFrame() {
  ...
  const uint32_t capture_width =
      static_cast<uint32_t>(capture_texture->width());
  ...
  blit->copyFromTexture(capture_texture, 0, 0, MTL::Origin(0, 0, 0),
                        MTL::Size(capture_width, capture_height, 1),
                        staging_buffer, 0, bytes_per_row, 0);
...
bool MetalCommandProcessor::IssueDraw(xenos::PrimitiveType primitive_type,
  ...
  bool tessellation_enabled = vgt_output_path_cntl.path_select ==
  ...
  if (res_heap_ab_ && smp_heap_ab_ && uniforms_buffer_ && shared_memory_) {
    ...
    gviargs.Setup(
        ...
        kViewportBoundsMax, kViewportBoundsMax,
        false,  // allow_reverse_z
        depth_control, convert_z_to_float24, host_render_targets_used,
...
MTL::CommandBuffer* MetalCommandProcessor::EnsureCommandBuffer() {
  ...
  current_command_buffer_->addCompletedHandler(
      [this, start_time](MTL::CommandBuffer*) {
        const auto end_time = std::chrono::steady_clock::now();
        const uint64_t elapsed_us =
            std::chrono::duration_cast<std::chrono::microseconds>(end_time -
                                                                  start_time)
                .count();
        completed_command_buffers_total_us_.fetch_add(
...
MetalCommandProcessor::GetOrCreateGeometryPipelineState(
    ...
    if (dump_geometry && geometry_dump_id >= 0) {
      ...
      std::string layout_json =
          layout_json_storage.empty()
              ...
              : layout_json_storage;
      DumpGeometryText(geometry_dump_dir, geometry_dump_id, geometry_dump_type,
                       geometry_dump_key, "vs_layout.json", layout_json);
  ...
  auto get_geometry_stage = [&]() -> GeometryShaderStageState* {
    ...
    if (dump_geometry && geometry_dump_id >= 0) {
      ...
      DumpGeometryArtifact(geometry_dump_dir, geometry_dump_id,
                           ...
                           dxil_data.data(), dxil_data.size());
      DumpGeometryArtifact(geometry_dump_dir, geometry_dump_id,
                           geometry_dump_type, geometry_dump_key, "gs.metallib",
      ...
      info << "gs_max_input_primitives_per_mesh_threadgroup="
           ...
           << "\n";
      DumpGeometryText(geometry_dump_dir, geometry_dump_id, geometry_dump_type,
                       geometry_dump_key, "gs_info.txt", info.str());
    }
  ...
  if (!pipeline) {
        ...
        error ? error->localizedDescription()->utf8String() : "unknown error");
    {
      ...
      if (!logged_materialize_probe && geometry_stage &&
          geometry_stage->library) {
        logged_materialize_probe = true;
        NS::Error* probe_error = nullptr;
        MTL::Function* probe_no_constants =
            geometry_stage->library->newFunction(NS::String::string(
                geometry_stage->function_name.c_str(), NS::UTF8StringEncoding));
        if (!probe_no_constants && probe_error) {
          LogMetalErrorDetails(
              "Geometry pipeline probe: geometry function (no constants)",
              probe_error);
        } else if (probe_no_constants) {
          probe_no_constants->release();
        }

        // UInt4-by-index probe (matches MSC function-constant register-space
        // rule).
        ...
        probe_fc->setConstantValue(output_size_u4, MTL::DataTypeUInt4,
                                   NS::UInteger(1));
        probe_error = nullptr;
        MTL::Function* probe_uint4 = geometry_stage->library->newFunction(
    ...
    if (dump_geometry && geometry_dump_id >= 0) {
      XELOGE("Geometry pipeline failure: geom_dump_id={} type={} key={:#010x}",
    ...
    auto log_materialization = [&]() {
      ...
      if (vertex_stage->stage_in_library) {
        ...
        if (names && names->count()) {
          ...
          MTL::Function* fn = vertex_stage->stage_in_library->newFunction(name);
          if (!fn) {
            XELOGE(
...
MetalCommandProcessor::CreateDrawRingBuffers() {
  ...
  TrackMetalBufferCreated(kResourceHeapBytes);
  ...
  TrackMetalBufferCreated(kSamplerHeapBytes);
  ...
  TrackMetalBufferCreated(kUniformsBufferSize);
  ...
  TrackMetalBufferCreated(kTopLevelABTotalBytes);
  ...
  TrackMetalBufferCreated(kDrawArgsSize);
  ...
  TrackMetalBufferCreated(kCBVHeapBytes);
...
void MetalCommandProcessor::LogCacheStatsIfNeeded() {
  ...
  const double completed_avg_ms =
      completed_delta ? (double(completed_total_us_delta) / 1000.0) /
  ...
  if (last_command_buffer_complete_time_ !=
    ...
    ms_since_complete = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  resource_delta.heap_bytes_released -=
      last_resource_stats_.heap_bytes_released;
  ...
  const double heaps_live_mb = bytes_to_mb(resource_stats.heap_bytes_created -
  ...
  if (log_file.is_open()) {
    log_file << fmt::format(
        ...
        "rt_mb~{:.2f} shaders={} shader_mb={:.2f} pipelines={} "
        "geom_pipelines={} "
        ...
        bytes_to_mb(cache_texture_bytes_created_delta), cb_state, enc_state,
        inflight, completed, ms_since_complete, created, completed_delta,
        completed_avg_ms, completed_max_ms, pools_created_delta,
        pools_drained_delta, scaled_resolve_count,
        ...
        resource_delta.buffers_created, buffers_created_mb, buffers_released_mb,
