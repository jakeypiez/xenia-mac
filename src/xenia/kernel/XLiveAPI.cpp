...
void XLiveAPI::SetAPIAddress(std::string address) {
  ...
  // If already initialized, reset and re-initialize with new server
...
void XLiveAPI::Init() {
  ...
  if (cvars::logging) {
    ...
    if (vinfo->features & CURL_VERSION_SSL) {
      ...
      XELOGW("No SSL support in libcurl!");
      XELOGI("No SSL");
    }
  }
...
void XLiveAPI::DownloadPortMappings() {
  ...
  if (response->StatusCode() != HTTP_STATUS_CODE::HTTP_OK) {
    // assert_always(); // Removed: crash in Debug builds
    return;
  }
...
std::unique_ptr<HTTPResponseObjectJSON> XLiveAPI::RegisterPlayer() {
  ...
  if (!mac_address_) {
    XELOGE("RegisterPlayer: mac_address_ is null!");
    return response;
  }
...
#else
  // On macOS, FriendlyName is already a char*, just use AdapterName
...
void XLiveAPI::DiscoverNetworkInterfaces() {
  ...
  if (getifaddrs(&ifaddr) == -1) {
    XELOGI("Failed to get network interfaces: {}", strerror(errno));
    return;
  }

  std::string networks = "Network Interfaces:\n";

  // We need stable storage for the unicast address structs since
  // IP_ADAPTER_ADDRESSES holds raw pointers to them.
  static std::vector<std::unique_ptr<IP_ADAPTER_UNICAST_ADDRESS_LH>>
      unicast_storage;
  static std::vector<std::unique_ptr<sockaddr_in>> sockaddr_storage;
  unicast_storage.clear();
  sockaddr_storage.clear();

  // Build a map of interface name -> adapter so we consolidate addresses
  std::map<std::string, size_t> iface_map;
  ...
  for (struct ifaddrs* ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    // Create or find the adapter entry
    if (iface_map.find(name) == iface_map.end()) {
      IP_ADAPTER_ADDRESSES adapter{};
      strncpy(adapter.AdapterName, name.c_str(),
              MAX_ADAPTER_NAME_LENGTH - 1);
      adapter.AdapterName[MAX_ADAPTER_NAME_LENGTH - 1] = '\0';
      adapter.FriendlyName = adapter.AdapterName;  // Same as name on macOS
      adapter.FirstUnicastAddress = nullptr;
      adapter.Next = nullptr;

      iface_map[name] = adapter_addresses.size();
      adapter_addresses.push_back(adapter);
    }

    size_t idx = iface_map[name];

    // Create a unicast address entry with stable storage
    auto sa = std::make_unique<sockaddr_in>(*sin);
    auto ua = std::make_unique<IP_ADAPTER_UNICAST_ADDRESS_LH>();
    ua->Address.lpSockaddr = reinterpret_cast<struct sockaddr*>(sa.get());
    ua->Next = adapter_addresses[idx].FirstUnicastAddress;
    adapter_addresses[idx].FirstUnicastAddress = ua.get();

    sockaddr_storage.push_back(std::move(sa));
    unicast_storage.push_back(std::move(ua));
    ...
    networks +=
        fmt::format("{} {}: {}\n", friendlyName, guid, ip_to_string(addr));
  }
