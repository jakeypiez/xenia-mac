...
// Cross-platform IP address byte extraction
// Windows uses S_un.S_un_b.s_b1-4, macOS/Linux use s_addr directly
#ifdef XE_PLATFORM_WIN32
#define IP_BYTE1(addr) (addr).S_un.S_un_b.s_b1
#define IP_BYTE2(addr) (addr).S_un.S_un_b.s_b2
#define IP_BYTE3(addr) (addr).S_un.S_un_b.s_b3
#define IP_BYTE4(addr) (addr).S_un.S_un_b.s_b4
#else
#define IP_BYTE1(addr) ((addr).s_addr & 0xFF)
#define IP_BYTE2(addr) (((addr).s_addr >> 8) & 0xFF)
#define IP_BYTE3(addr) (((addr).s_addr >> 16) & 0xFF)
#define IP_BYTE4(addr) (((addr).s_addr >> 24) & 0xFF)
#endif
...
dword_result_t NetDll_WSARecvFrom_entry(
  ...
  if (ret < 0) {
    ...
    XELOGI("NetDll_WSARecvFrom: Received {} bytes from: {}.{}.{}.{}",
           ...
           IP_BYTE1(from_ptr->address_ip),
           IP_BYTE2(from_ptr->address_ip),
           IP_BYTE3(from_ptr->address_ip),
           IP_BYTE4(from_ptr->address_ip));
  }
...
dword_result_t NetDll_WSASendTo_entry(
  ...
  if (result == -1) {
    ...
    XELOGI("NetDll_WSASendTo: Send {} bytes to: {}.{}.{}.{}", result,
           IP_BYTE1(to_ptr->address_ip),
           IP_BYTE2(to_ptr->address_ip),
           IP_BYTE3(to_ptr->address_ip),
           IP_BYTE4(to_ptr->address_ip));
  }
...
dword_result_t NetDll_XNetDnsLookup_entry(dword_t caller, lpstring_t host,
  ...
  if (event_handle) {
    ...
    if (ev) {
      ...
      XELOGW("NetDll_XNetDnsLookup: event handle {:08X} not found", event_handle.value());
    }
  }
  return 0;
}
...
dword_result_t NetDll_XNetQosServiceLookup_entry(dword_t caller, dword_t flags,
  ...
  if (event_handle) {
    ...
    if (ev) {
      ...
      XELOGW("NetDll_XNetQosServiceLookup: event handle {:08X} not found", event_handle.value());
    }
  }
...
dword_result_t NetDll_XNetQosListen_entry(
  ...
  if (data_size > (uint32_t)(xnet_startup_params.cfgQosDataLimitDiv4 * 4)) {
    XELOGW("XNetQosListen: data_size {} exceeds QoS limit {}", data_size.value(),
           xnet_startup_params.cfgQosDataLimitDiv4 * 4);
  ...
  */
  ...
  if (event_handle) {
    ...
    if (ev) {
      ...
      XELOGW("NetDll_XNetQosLookup: event handle {:08X} not found", event_handle.value());
    }
  }
  ...
  void Load(const x_fd_set* guest_set) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      auto socket =
          kernel_state()->object_table()->LookupObject<XSocket>(socket_handle);
      if (!socket) {
        XELOGW("fd_set Load: socket handle {:08X} not found", socket_handle);
        count = i;
        break;
      }
      sockets[i] = socket;
...
dword_result_t NetDll_WSAEventSelect_entry(dword_t caller,
  ...
  // XEvent doesn't expose a native handle on macOS; pass 0 since
  // WSAEventSelect is a no-op stub on non-Windows platforms.
  int ret = socket->WSAEventSelect(socket->native_handle(), 0, flags);
