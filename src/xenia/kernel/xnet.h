...
struct XSESSION_MEMBER {
  ...
  void SetPrivate() {
    Flags = static_cast<uint32_t>(Flags) | static_cast<uint32_t>(MEMBER_FLAGS::PRIVATE_SLOT);
  }

  void SetZombie() { Flags = static_cast<uint32_t>(Flags) | static_cast<uint32_t>(MEMBER_FLAGS::ZOMBIE); }

  const bool IsPrivate() const {
    return (static_cast<uint32_t>(Flags) & static_cast<uint32_t>(MEMBER_FLAGS::PRIVATE_SLOT)) ==
           static_cast<uint32_t>(MEMBER_FLAGS::PRIVATE_SLOT);
  }

  const bool IsZombie() const {
    return (static_cast<uint32_t>(Flags) & static_cast<uint32_t>(MEMBER_FLAGS::ZOMBIE)) ==
